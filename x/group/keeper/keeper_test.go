...
import (
	...
	"github.com/vishal-kanna/group/x/group/internal/math"
	"github.com/vishal-kanna/group/x/group/keeper"
...
func (s *TestSuite) TestProposalsByVPEnd() {
	...
	specs := map[string]struct {
		...
		"tally updated after voting period end": {
			preRun: func(sdkCtx sdk.Context) uint64 {
				return submitProposal(ctx.Context(), s, []sdk.Msg{msgSend}, proposers)
			},
			admin:     proposers[0],
	...
	for msg, spec := range specs {
		...
		s.Run(msg, func() {
			...
			resp, err := s.keeper.Proposal(ctx.Context(), &group.QueryProposalRequest{
				ProposalId: pId,
			})
