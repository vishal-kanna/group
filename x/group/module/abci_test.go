...
import (
	...
	fun "github.com/vishal-kanna/group/simapp"
	"github.com/vishal-kanna/group/x/group"

	sdk "github.com/cosmos/cosmos-sdk/types"

	banktypes "github.com/cosmos/cosmos-sdk/x/bank/types"
	"github.com/vishal-kanna/group/simapp"

	// "github.com/cosmos/cosmos-sdk/x/group/module"
	"github.com/vishal-kanna/group/x/group/module"
...
func TestEndBlockerPruning(t *testing.T) {
	...
	a := sdk.WrapSDKContext(ctx)
	groupRes, err := app.GroupKeeper.CreateGroup(a, &group.MsgCreateGroup{
		Admin:   addr1.String(),
		Members: members,
	})
	require.NoError(t, err)

	groupRes2, err := app.GroupKeeper.CreateGroup(a, &group.MsgCreateGroup{
		Admin:   addr2.String(),
		Members: members,
	})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	policyRes, err := app.GroupKeeper.CreateGroupPolicy(a, policyReq)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	policyRes2, err := app.GroupKeeper.CreateGroupPolicy(a, policyReq2)
	require.NoError(t, err)

	groupPolicyAddr, err := sdk.AccAddressFromBech32(policyRes.Address)
	require.NoError(t, err)
	require.NoError(t, fun.FundAccount(app.BankKeeper, ctx, groupPolicyAddr, sdk.Coins{sdk.NewInt64Coin("test", 10000)}))

	groupPolicyAddr2, err := sdk.AccAddressFromBech32(policyRes2.Address)
	require.NoError(t, err)
	require.NoError(t, fun.FundAccount(app.BankKeeper, ctx, groupPolicyAddr2, sdk.Coins{sdk.NewInt64Coin("test", 10000)}))
	...
	specs := map[string]struct {
		...
		"proposal pruned after executor result success": {
			setupProposal: func(ctx sdk.Context) uint64 {
				...
				pID, err := submitProposalAndVote(app, a, msgs, proposers, groupPolicyAddr, group.VOTE_OPTION_YES)
				require.NoError(t, err)
				_, err = app.GroupKeeper.Exec(a, &group.MsgExec{Executor: addr3.String(), ProposalId: pID})
				require.NoError(t, err)
				sdkCtx := sdk.UnwrapSDKContext(a)
				require.NoError(t, fun.FundAccount(app.BankKeeper, sdkCtx, groupPolicyAddr, sdk.Coins{sdk.NewInt64Coin("test", 10002)}))
		...
		"proposal with multiple messages pruned when executed with result success": {
			setupProposal: func(ctx sdk.Context) uint64 {
				...
				_, err = app.GroupKeeper.Exec(a, &group.MsgExec{Executor: addr3.String(), ProposalId: pID})
				require.NoError(t, err)
				require.NoError(t, fun.FundAccount(app.BankKeeper, ctx, groupPolicyAddr, sdk.Coins{sdk.NewInt64Coin("test", 10002)}))
		...
		"proposal not pruned when not executed and rejected": {
			setupProposal: func(ctx sdk.Context) uint64 {
				msgs := []sdk.Msg{msgSend1}
				pID, err := submitProposalAndVote(app, a, msgs, proposers, groupPolicyAddr, group.VOTE_OPTION_NO)
				require.NoError(t, err)
				_, err = app.GroupKeeper.Exec(a, &group.MsgExec{Executor: addr3.String(), ProposalId: pID})
				require.NoError(t, err)
				require.NoError(t, fun.FundAccount(app.BankKeeper, ctx, groupPolicyAddr, sdk.Coins{sdk.NewInt64Coin("test", 10002)}))
		...
		"open proposal is not pruned which must not fail ": {
			setupProposal: func(ctx sdk.Context) uint64 {
				pID, err := submitProposal(app, a, []sdk.Msg{msgSend1}, proposers, groupPolicyAddr)
				require.NoError(t, err)
				_, err = app.GroupKeeper.Exec(a, &group.MsgExec{Executor: addr3.String(), ProposalId: pID})
				require.NoError(t, err)
				require.NoError(t, fun.FundAccount(app.BankKeeper, ctx, groupPolicyAddr, sdk.Coins{sdk.NewInt64Coin("test", 10002)}))
		...
		"proposal not pruned with group policy modified before tally": {
			setupProposal: func(ctx sdk.Context) uint64 {
				pID, err := submitProposal(app, a, []sdk.Msg{msgSend1}, proposers, groupPolicyAddr)
				require.NoError(t, err)
				_, err = app.GroupKeeper.UpdateGroupPolicyMetadata(a, &group.MsgUpdateGroupPolicyMetadata{
					Admin:              addr1.String(),
		...
		"pruned when proposal is executable when failed before": {
			setupProposal: func(ctx sdk.Context) uint64 {
				...
				pID, err := submitProposalAndVote(app, a, msgs, proposers, groupPolicyAddr, group.VOTE_OPTION_YES)
				require.NoError(t, err)
				_, err = app.GroupKeeper.Exec(a, &group.MsgExec{Executor: addrs[2].String(), ProposalId: pID})
				require.NoError(t, err)
				return pID
			},
			newCtx:            ctx,
		...
		"proposal with status withdrawn is pruned after voting period end": {
			setupProposal: func(sdkCtx sdk.Context) uint64 {
				pId, err := submitProposal(app, sdk.WrapSDKContext(sdkCtx), []sdk.Msg{msgSend1}, proposers, groupPolicyAddr)
				require.NoError(t, err)
				_, err = app.GroupKeeper.WithdrawProposal(sdk.WrapSDKContext(sdkCtx), &group.MsgWithdrawProposal{
					ProposalId: pId,
					Address:    proposers[0],
				})
				require.NoError(t, err)
		...
		"proposal with status aborted is pruned after voting period end (due to updated group policy decision policy)": {
			setupProposal: func(sdkCtx sdk.Context) uint64 {
				pId, err := submitProposal(app, sdk.WrapSDKContext(sdkCtx), []sdk.Msg{msgSend2}, proposers, groupPolicyAddr2)
				require.NoError(t, err)
				...
				require.NoError(t, err)
				_, err = app.GroupKeeper.UpdateGroupPolicyDecisionPolicy(sdk.WrapSDKContext(sdkCtx), msg)
				require.NoError(t, err)
	...
	for msg, spec := range specs {
		...
		t.Run(msg, func(t *testing.T) {
			...
			if spec.expErrMsg != "" && spec.expExecutorResult != group.PROPOSAL_EXECUTOR_RESULT_SUCCESS {
				_, err = app.GroupKeeper.Proposal(sdk.WrapSDKContext(spec.newCtx), &group.QueryProposalRequest{ProposalId: proposalID})
				require.Error(t, err)
			...
			if spec.expExecutorResult == group.PROPOSAL_EXECUTOR_RESULT_SUCCESS {
				...
				_, err = app.GroupKeeper.Proposal(sdk.WrapSDKContext(spec.newCtx), &group.QueryProposalRequest{ProposalId: proposalID})
				require.Contains(t, err.Error(), spec.expErrMsg)
				res, err := app.GroupKeeper.VotesByProposal(sdk.WrapSDKContext(ctx), &group.QueryVotesByProposalRequest{ProposalId: proposalID})
				require.NoError(t, err)
				require.Empty(t, res.GetVotes())
			} else {
				// Check that proposal and votes exists
				res, err := app.GroupKeeper.Proposal(sdk.WrapSDKContext(spec.newCtx), &group.QueryProposalRequest{ProposalId: proposalID})
				require.NoError(t, err)
				_, err = app.GroupKeeper.VotesByProposal(sdk.WrapSDKContext(ctx), &group.QueryVotesByProposalRequest{ProposalId: res.Proposal.Id})
				require.NoError(t, err)
...
func TestEndBlockerTallying(t *testing.T) {
	...
	groupRes, err := app.GroupKeeper.CreateGroup(sdk.WrapSDKContext(ctx), &group.MsgCreateGroup{
	...
	err = policyReq.SetDecisionPolicy(policy)
	require.NoError(t, err)
	policyRes, err := app.GroupKeeper.CreateGroupPolicy(sdk.WrapSDKContext(ctx), policyReq)
	require.NoError(t, err)
	...
	specs := map[string]struct {
		...
		"tally updated after voting period end": {
			preRun: func(sdkCtx sdk.Context) uint64 {
				pId, err := submitProposal(app, sdk.WrapSDKContext(sdkCtx), []sdk.Msg{msgSend}, proposers, groupPolicyAddr)
				require.NoError(t, err)
				return pId
			},
			admin:     proposers[0],
		...
		"tally within voting period(with votes)": {
			preRun: func(sdkCtx sdk.Context) uint64 {
				pId, err := submitProposalAndVote(app, sdk.WrapSDKContext(sdkCtx), []sdk.Msg{msgSend}, proposers, groupPolicyAddr, group.VOTE_OPTION_YES)
				require.NoError(t, err)
		...
		"tally after voting period (not passing)": {
			preRun: func(sdkCtx sdk.Context) uint64 {
				...
				pId, err := submitProposalAndVote(app, sdk.WrapSDKContext(sdkCtx), []sdk.Msg{msgSend}, []string{addrs[1].String()}, groupPolicyAddr, group.VOTE_OPTION_YES)
				require.NoError(t, err)
		...
		"tally of withdrawn proposal (with votes)": {
			preRun: func(sdkCtx sdk.Context) uint64 {
				pId, err := submitProposalAndVote(app, sdk.WrapSDKContext(ctx), []sdk.Msg{msgSend}, proposers, groupPolicyAddr, group.VOTE_OPTION_YES)
				require.NoError(t, err)

				_, err = app.GroupKeeper.WithdrawProposal(sdk.WrapSDKContext(ctx), &group.MsgWithdrawProposal{
					ProposalId: pId,
					Address:    proposers[0],
				})
	...
	for msg, spec := range specs {
		t.Run(msg, func(t *testing.T) {
			...
			module.EndBlocker(spec.newCtx, app.GroupKeeper)
			resp, err := app.GroupKeeper.Proposal(sdk.WrapSDKContext(spec.newCtx), &group.QueryProposalRequest{
				ProposalId: pId,
			})
